...
fn test_simulation_service_restart_simulation() -> Result<()> {
    ...
    // Modify the simulation to have a different state
    ...
    assert_eq!(simulation.ip(), ip);
    assert_eq!(simulation.id().value(), id.value());
    assert_eq!(simulation.balance().clear_value(), 1000); // Should be reset to config balance
    assert_eq!(simulation.round(), 0); // Should be reset to 0
...
fn test_simulation_service_create_round() -> Result<()> {
    ...
    // Check that games were actually created in the database
